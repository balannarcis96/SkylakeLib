...
namespace SKL
{
    ...
    public:
        ...
        SKL_NODISCARD constexpr SKL_ALLOCATOR_FUNCTION T* allocate( const size_t InCount ) noexcept 
        {
            ...
            {
                GLOG_DEBUG( "STLAllocator<T>::Allocate() Failed to allocate %u bytes (%llu items)", AllocateSize, InCount );
                return nullptr;
            }
    ...
    public:
        ...
        SKL_NODISCARD constexpr SKL_ALLOCATOR_FUNCTION T* allocate( const size_t InCount ) noexcept 
        {
            ...
            if( false == AllocResult.IsValid() ) SKL_UNLIKELY
            {
                GLOG_DEBUG( "STLTLSAllocator<T>::Allocate() Failed to allocate %u bytes (%llu items)", AllocateSize, InCount );
                return nullptr;
            }
